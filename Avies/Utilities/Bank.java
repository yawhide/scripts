...
public class Bank {
	...
	public static void bank() {
		...
		withdraw(
				Avies.foodNumber + ((Combat.getMaxHP() - Combat.getHP()) / 12),
				Avies.foodIDs);
		...
		if (currP < pt * 2) {
			...
			if (!Banking.isBankScreenOpen()) {
				if (Banking.openBankBanker()) {
					...
					Banking.depositAll();
					Conditionals.waitFor(Inventory.getAll().length == 0, 400,
							500);
				}
			}
		}
	...
	public static void updateItemCountAtBank(){
